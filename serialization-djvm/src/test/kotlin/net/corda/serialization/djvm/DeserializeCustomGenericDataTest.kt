...
class DeserializeCustomGenericDataTest: TestBase(KOTLIN) {
    ...
    @RegisterExtension
    @JvmField
    val serialization = LocalSerialization(setOf(CustomSerializer()))
    ...
    fun `test deserializing custom generic object`() {
        ...
        sandbox {
            _contextSerializationEnv.set(createSandboxSerializationEnv(classLoader, setOf(CustomSerializer::class.java.name)))
